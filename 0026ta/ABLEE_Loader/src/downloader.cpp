...
void CDownloader::SlotDownloadFile()
{
	...
	if( !m_bFtpInfo )
	{
		emit SigDownloadResult( false, tr("No FTP connection information.") );
		return;
	}
	...
	{
		emit SigDownloadResult( false, tr("FTP download start failed!") );
	}
}
...
void CDownloader::SlotReplyFinished()
{
	if( m_pReply->error() != QNetworkReply::NoError )
	{
		...
		emit SigDownloadResult( false, m_sErrMsg );
		m_pFile->close();
		return;
	}
	...
	m_pFile->close();
	m_pFile->deleteLater();
	m_sErrMsg = "Downlaod done.";
	emit SigDownloadResult( true, m_sErrMsg );
